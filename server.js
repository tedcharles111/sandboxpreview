...
// ----------------------------------------------------------------------
// Serve preview page – build HTML with safe concatenation (no nested backticks)
app.get('/preview/:id', (req, res) => {
  ...
  // Build the HTML response using an array to avoid template literal pitfalls
  const htmlParts = [
    '<!DOCTYPE html>',
    '<html lang="en">',
    '<head>',
    ...
    '#toolbar { background: #1e1e2f; color: white; padding: 8px 16px; font-size: 13px; display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid #333; }',
    ...
    '#error { position: fixed; bottom: 20px; left: 20px; right: 20px; background: #ff4444cc; backdrop-filter: blur(8px); color: white; padding: 12px; border-radius: 8px; font-family: monospace; font-size: 13px; z-index: 1000; display: none; max-height: 200px; overflow: auto; border-left: 4px solid #ff0000; }',
    '.loading-overlay { position: absolute; top: 0; left: 0; right: 0; bottom: 0; background: #0f1117; display: flex; align-items: center; justify-content: center; flex-direction: column; gap: 16px; z-index: 100; transition: opacity 0.3s ease; }',
    '.spinner { width: 40px; height: 40px; border: 3px solid rgba(99,102,241,0.3); border-top-color: #6366f1; border-radius: 50%; animation: spin 0.8s linear infinite; }',
    ...
    '    } else { rewritten.set(path, content); }',
    '  }',
    '  const fsPlugin = {',
    '    name: "fs",',
    '    setup(build) {',
    '      build.onResolve({ filter: /.*/ }, args => {',
    '        if (args.path.startsWith(".") || args.path.startsWith("/")) { return { path: args.path, namespace: "file" }; }',
    ...
    '      bundle: true, write: false, format: "iife", globalName: "SandboxModule",',
    '      platform: "browser", plugins: [fsPlugin],',
    '      loader: { ".js": "jsx", ".ts": "tsx", ".tsx": "tsx" },',
    '      define: { "process.env.NODE_ENV": "\\"development\\"" }',
    '    });',
    '    const bundledCode = result.outputFiles[0].text;',
    '    const reactCDNs = \'<script src="https://cdn.jsdelivr.net/npm/react@18.2.0/umd/react.development.js"><\\/script>\' +',
    ...
    '    const iframeHtml = "<!DOCTYPE html><html><head><meta charset=\\"UTF-8\\"><title>Preview</title>" + reactCDNs +',
